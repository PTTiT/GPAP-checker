...
User can click on the setting button in the top bar of GPAP Checker screen to navigate to the import screen.
After user imports correctly, it backs to the GPAP Checker screen. The GPAP Checker screen is refreshed with the updated app list.
...
A user wants to quickly add a few custom package names to check without creating a file. They navigate to the import screen, paste comma-separated package names into a text field, and submit. The app validates the input, imports the packages, and updates the GPAP Checker screen to show the newly added apps.
...
**Independent Test**: Can be fully tested by entering "com.example.app1, com.example.app2" in the text input field and verifying that the GPAP Checker screen displays these two packages in the app list.

**Acceptance Scenarios**:

1. **Given** user is on the GPAP Checker screen, **When** user taps the settings button in the top bar, **Then** the app navigates to the import screen
2. **Given** user is on the import screen with an empty text field, **When** user enters "com.package.one, com.package.two" and submits, **Then** the app validates the input, saves the packages, returns to the GPAP Checker screen, and displays both packages in the app list
...
A user has a prepared text file containing package names (one per line or comma-separated). They navigate to the import screen, select a .txt file from their device storage, and import it. The app parses the file, validates the package names, and updates the GPAP Checker screen with the imported list.
...
**Independent Test**: Can be fully tested by creating a .txt file with package names, selecting it via the file picker, and verifying that all packages from the file appear in the GPAP Checker screen's app list.

**Acceptance Scenarios**:

1. **Given** user is on the import screen, **When** user taps "Import from File" and selects a .txt file containing package names, **Then** the app reads the file, parses package names, and navigates back to the GPAP Checker screen with the imported packages displayed
...
**Independent Test**: Can be fully tested by importing an initial list, then importing a new list with the "Replace" option, and verifying that only the new packages appear in the GPAP Checker screen.
...
- **Navigate away without importing**: Changes discarded, return to GPAP Checker screen with original list (Assumption 7)
...
- **FR-001**: System MUST provide a settings button in the top bar of the GPAP Checker screen that navigates to the import screen
...
- **FR-012**: System MUST navigate back to the GPAP Checker screen after successful import
- **FR-013**: System MUST refresh the GPAP Checker screen to display the updated app list after import
...
- **SC-001**: Users can navigate from the GPAP Checker screen to the import screen with a single tap on the settings button
...
- **SC-005**: The GPAP Checker screen displays the updated app list immediately after import without requiring manual refresh
...
7. **Navigation**: The back button on the import screen will return to the GPAP Checker screen without importing (cancel behavior).
